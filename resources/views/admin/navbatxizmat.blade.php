    ...
    if (selectedIds.length === 0) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#suss_message').fadeIn().delay(1200).fadeOut();
                setTimeout(function() {
                    window.location.href = "{{ route('adminnavbatxizmat') }}";
                }, 4000);
            },
            error: function(xhr, status, error) {
                // Handle error
...
<!-- /.form-group -->
    ...
    <div class="card-header">
        <h3 class="card-title">Xizmat Ko`rsatilganlarni ko`rish</h3>
    </div>
    ...
    <button style="float: left; margin-right: 1%; width: 200px;" type="button" id="submit2" class="btn btn-danger" onclick="deleteSelectedItems()">O`chirish</button>
</div>
